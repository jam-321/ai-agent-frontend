...
async function create() {
  if (props.mode === 'agents') { editing.value = {}; editingExisting.value = false; form.value = { agentKey: '', adminOnly: false, executionType: 'LOOP', modelProviderKey: 'deepseek', modelName: 'deepseek-v4-flash', modelTemperature: 0.7, enabledPlugins: '[]', enabledTools: 'null', magicParams: '{}', imageHistoryMode: 'SUMMARY_TOOL' }; return }
...
<template>
  <main class="admin-page"><div class="page-toolbar"><div><h2>{{ title }}</h2><span>敏感凭据只显示配置状态，不返回明文</span></div><div><button v-if="mode === 'agents' || mode === 'providers'" @click="create">新建</button><button @click="load">刷新</button></div></div><p v-if="error" class="page-error">{{ error }}</p>
    ...
    <div v-else-if="mode === 'agents'" class="admin-table-wrap"><table><thead><tr><th>Agent</th><th>访问范围</th><th>执行类型</th><th>默认模型</th><th>温度</th><th>工具</th><th>操作</th></tr></thead><tbody><tr v-for="row in rows" :key="row.agentKey"><td>{{ row.agentKey }}</td><td>{{ row.adminOnly ? '仅管理员' : '普通用户可用' }}</td><td>{{ row.executionType }}</td><td>{{ row.modelProviderKey }} / {{ row.modelName }}</td><td>{{ row.modelTemperature }}</td><td>{{ row.enabledTools == null ? '全部' : [...row.enabledTools].join(', ') }}</td><td><button class="text-button" @click="edit(row)">编辑</button><button v-if="row.agentKey !== 'general' && row.agentKey !== 'system_admin'" class="text-button danger" @click="remove(row)">删除</button></td></tr></tbody></table></div>
